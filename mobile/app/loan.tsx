...
export default function LoanScreen() {
  ...
  const validateStep = (step: number): boolean => {
    switch (step) {
      case 1:
        return !!(formData.fullName && formData.email && formData.phone && formData.pan);
      case 2:
    ...
    try {
      ...
      if (result.success) {
        Alert.alert(
          ...
          'We have received your loan application and will review it shortly.',
          ...
          <View className="space-y-4">
                  ...
                  {['Sole Proprietorship', 'Partnership', 'Private Limited', 'Other'].map((type) => (
                    <TouchableOpacity
                      ...
                      className={`px-4 py-2 rounded-full mr-2 ${
                        formData.businessType === type.toLowerCase().replace(' ', '-') 
                          ? 'bg-primary' 
                          : 'bg-secondary'
                      }`}
                      onPress={() => updateFormData('businessType', type.toLowerCase().replace(' ', '-'))}
                    >
            ...
            <View>
              ...
              <TextInput
                className="border border-input bg-background px-3 py-3 rounded-md text-foreground"
                placeholder="e.g., 5"
                value={formData.yearsInBusiness > 0 ? formData.yearsInBusiness.toString() : ''}
                onChangeText={(text) => updateFormData('yearsInBusiness', parseInt(text) || 0)}
            ...
            <View>
              ...
              <TextInput
                ...
                value={formData.annualTurnover > 0 ? formData.annualTurnover.toString() : ''}
                onChangeText={(text) => updateFormData('annualTurnover', parseInt(text) || 0)}
                keyboardType="numeric"
              />
            </View>
          </View>
          ...
          <View className="space-y-4">
            <View>
              ...
              <TextInput
                ...
                value={formData.loanAmount > 0 ? formData.loanAmount.toString() : ''}
                onChangeText={(text) => updateFormData('loanAmount', parseInt(text) || 0)}
                keyboardType="numeric"
              />
            </View>
            <View>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} className="px-3 py-3">
                  {['Working Capital', 'Business Expansion', 'Equipment Purchase', 'Other'].map((purpose) => (
                    <TouchableOpacity
                      key={purpose}
                      className={`px-4 py-2 rounded-full mr-2 ${
                        formData.loanPurpose === purpose.toLowerCase().replace(' ', '-') 
                          ? 'bg-primary' 
                          : 'bg-secondary'
                      }`}
                      onPress={() => updateFormData('loanPurpose', purpose.toLowerCase().replace(' ', '-'))}
                    >
                      <Text className={`text-sm ${
                        formData.loanPurpose === purpose.toLowerCase().replace(' ', '-')
                          ? 'text-primary-foreground'
        ...
        <View className="py-6">
          <View className="bg-card p-6 rounded-lg shadow-sm">
            ...
            <View className="flex-row justify-between mt-8">
              {currentStep > 1 ? (
                <TouchableOpacity
                  className="flex-row items-center border border-border py-3 px-6 rounded-lg"
                  onPress={handlePrev}
                >
